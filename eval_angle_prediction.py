...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.out_dim = 4 if args.experience == "quat" else 3
    ...
    def forward(self, x, y):
        if not self.inv:
            ...
            out_x = self.backbone(x)[..., :-self.equi_dims]
            out_y = self.backbone(y)[..., :-self.equi_dims]
        concat = torch.cat([out_x, out_y], dim=1)
...
class Dataset3DIEBench(Dataset):
    ...
    def __getitem__(self, i):
        ...
        sample = Path(str(self.samples[i]))  # numpy.str_ -> str -> Path
        folder = f"{str(self.dataset_root)}/{str(sample)}"

        img_1 = self.get_img(f"{folder}/image_{int(views[0])}.jpg")
        img_2 = self.get_img(f"{folder}/image_{int(views[1])}.jpg")

        angles_1 = np.load(f"{folder}/latent_{int(views[0])}.npy")[:3].astype(np.float32)
        angles_2 = np.load(f"{folder}/latent_{int(views[1])}.npy")[:3].astype(np.float32)
...
def load_from_state_dict(model, state_dict, prefix, new_suffix):
    ...
    for k, v in model.state_dict().items():
        if k not in list(state_dict):
            ...
            print('key "{}" is of different shape in model and provided state dict {} vs {}'.format(
...
### DATA
ds_train = Dataset3DIEBench(
    args.dataset_root,
    "./data/train_images.npy",
    args.experience,
    transform=transforms.Compose([transforms.ToTensor(), normalize])
)
ds_val = Dataset3DIEBench(
    args.dataset_root,
    "./data/val_images.npy",
    args.experience,
    transform=transforms.Compose([transforms.ToTensor(), normalize])
    ...
    with torch.no_grad():
        ...
        for i, (inputs_1, inputs_2, latents) in enumerate(val_loader):
            ...
            if i == 0:
                ...
                total_latents = torch.cat((total_latents, latents.cpu()), dim=0)
                total_preds = torch.cat((total_preds, outputs.cpu()), dim=0)
